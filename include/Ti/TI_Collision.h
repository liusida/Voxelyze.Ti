...
{
public:
	CUDA_CALLABLE_MEMBER TI_Collision(TI_Voxel* v1, TI_Voxel* v2); //!< Constructor taking the two voxels to watch for collision between. The order is irrelevant. @param[in] v1 One voxel @param[in] v2 The other voxel
	CUDA_CALLABLE_MEMBER TI_Collision& operator=(const TI_Collision& col); //!< Overload "=" operator.
	CUDA_CALLABLE_MEMBER TI_Collision(const TI_Collision& col) {*this = col;} //!< copy constructor.

	CUDA_CALLABLE_MEMBER TI_Vec3D<float> const contactForce(TI_Voxel* pVoxel); //!< Returns the repelling force acting on pVoxel from the penetration of the other voxel if their collision boundaries overlap. Otherwise returns a zero force. This force will only be accurate if updateContactForce() has been called since the voxels last moved. @param[in] pVoxel The voxel in question. This should be voxel1() or voxel2() to have any meaning. Otherwise a zero force is returned.
	CUDA_CALLABLE_MEMBER void updateContactForce(); //!< Updates the state this collision based on the current positions and properties of voxel1() and voxel2(). Call contactForce() with either voxel as the argument to obtain the repelling penetration force (if any) that exisits.

	CUDA_CALLABLE_MEMBER TI_Voxel* voxel1() const {return pV1;} //!<One voxel of this potential collision pair.
	CUDA_CALLABLE_MEMBER TI_Voxel* voxel2() const {return pV2;} //!<The other voxel of this potential collision pair.
